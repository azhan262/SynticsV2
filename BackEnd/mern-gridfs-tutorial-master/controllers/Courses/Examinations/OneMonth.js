...
var { SectionsExaminationsOneMonth } = require('../../../models/Courses/Examinations/OneMonth')
 ...
 router.post('/examination-one-month-create-sections', (req, res) => {
    Student.find({"courses" : "Examination / One-One / Month" },(err, doc) => {
        if (!err) {
            var count = 0
            ...
            doc.forEach(element => {
             ...
             sectionArray.forEach(element => {
                ...
                var emp = new SectionsExaminationsOneMonth({
...
router.post('/examination-one-month-deletefields', (req, res) => {
