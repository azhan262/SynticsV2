...
var { SectionsJuniorOneMonth} = require('../../../models/Courses/Junior/JuniorOneMonth')
 ...
 router.post('/junior-one-month-create-sections', (req, res) => {
    Student.find({"courses" : "Junior / One-One / Month" },(err, doc) => {
        if (!err) {
            var count = 0
            ...
            doc.forEach(element => {
             ...
             sectionArray.forEach(element => {
                ...
                var emp = new SectionsJuniorOneMonth({
...
router.post('/junior-one-month-deletefields', (req, res) => {
