...
var { Questions } = require('../models/questions')
 ...
 router.post('/', (req, res) => {
     Questions.find((err, doc) => {
         if (!err) { res.send(doc); }
         else { console.log('Error in Retrieving Questionss :' + JSON.stringify(err, undifines, 2)); }
     });
 ...
 router.post('/create', (req, res) => {
     ...
     emp.save((err, doc) => {
         //Checking for error
         //if (!err) { res.send(doc);}
         //else {console.log('Error in Questions Save :' + JSON.stringify(err, undefined, 2)); }
     });
 ...
 router.put('/:id', (req, res) => {
    ...
    Questions.findByIdAndUpdate(req.params.id, { $set: emp}, { new: true }, (err, doc) =>{
        //Checking for errors
        //If error not found, sending response to the doc
        if(!err) {res.send(doc); }
        else { console.log('Error in Questionss Update :' + JSON.stringify(err, undefined, 2)); }
    });
 ...
 router.delete('/:id', (req, res) => {
    ...
    Questions.findByIdAndRemove(req.params.id, (err, doc) => {
        if (!err) { res.send(doc); }
        else { console.log('Error in Questionss Delete :' + JSON.stringify(err, undefined, 2)); }
    });
});
