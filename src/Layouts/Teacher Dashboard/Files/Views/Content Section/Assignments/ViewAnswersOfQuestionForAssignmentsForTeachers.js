...
import { getAnswerSpecificAssignment } from '../../../Apis/apiForAnswers';

function ViewAnswersOfQuestionForAssignmentsForTeachers() {
    let {id} = useParams()
    ...
    useEffect(() => {
      const fetchItems = async function() {
        const contents = await getAnswerSpecificAssignment(location.state._id)
    ...
    const handleOnClick = (data) => {
        ...
        if(( data.answertype == "Handwriting") || (data.answertype == "Drawing") || (data.answertype == "Audio") || (data.answertype == "Video") || (data.answertype == "Upload a File")){
          history.push({
            pathname:`/teacher/view-current-answer-attachments/${id}`,
            state: data
          })
        }
        else {
          history.push({
            pathname:`/teacher/view-current-answer-assignment/${id}`,
            state: data
          })
        }
    }
...
export default ViewAnswersOfQuestionForAssignmentsForTeachers
