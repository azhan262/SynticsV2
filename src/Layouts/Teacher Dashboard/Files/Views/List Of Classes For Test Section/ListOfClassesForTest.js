...
import { getTestPlanningTeacherSpecific } from '../../Apis/apiForTestContentPlanning'
...
function ListOfClassesForTestForTeacher() {
  ...
  useEffect(() => {
    const fetchItems = async function() {
      const contents = await getTestPlanningTeacherSpecific(id)
      setItems(contents)
    }
    fetchItems()
  }, []);
  ...
  const handleOnClickView = (data) => {
    if(data.answertype == "Audio"){
      ...
      history.push({
        pathname : `/teacher/view-current-content-file-upload-test/${id}`,
        state: data
      })
    }
    else{
