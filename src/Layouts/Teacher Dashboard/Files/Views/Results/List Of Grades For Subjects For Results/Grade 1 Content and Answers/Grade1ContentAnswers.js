...
import { getAnswersGrade1 } from '../../../../Apis/apiForAnswers';


function Grade1ContentAnswersForTeacher() {
    const history = useHistory()
    const [items, setItems] = useState([])
    useEffect(() => {
        const fetchItems = async function() {
          const contents = await getAnswersGrade1()
          setItems(contents)
        }
        fetchItems()
      }, []);
      ...
      const handleOnClickAnswers = (data) => {
        ...
        history.push({
            pathname:"/view-result-qa-answer-type",
            state: data
        })
      }
    return (
...
export default Grade1ContentAnswersForTeacher
