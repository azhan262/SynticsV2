...
import { getCoursePlanningsForGrade11 } from '../../../../Apis/apiForCoursePlanning';


function Grade11ContentForResultsForSubjectsForTeacher() {
  const [items, setItems] = useState([])
  const [answers, setAnswers] = useState([])

  useEffect(() => {
    const fetchItems = async function() {
      const contents = await getCoursePlanningsForGrade11()
      setItems(contents)
    }
    fetchItems()
  }, []);
...
export default Grade11ContentForResultsForSubjectsForTeacher
