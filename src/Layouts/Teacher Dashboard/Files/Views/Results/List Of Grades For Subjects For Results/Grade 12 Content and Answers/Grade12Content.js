...
import { getCoursePlanningsForGrade12 } from '../../../../Apis/apiForCoursePlanning';


function Grade12ContentForResultsForSubjectsForTeacher() {
  const [items, setItems] = useState([])
  const [answers, setAnswers] = useState([])

  useEffect(() => {
    const fetchItems = async function() {
      const contents = await getCoursePlanningsForGrade12()
      setItems(contents)
    }
    fetchItems()
  }, []);
...
export default Grade12ContentForResultsForSubjectsForTeacher
