...
function Grade2ContentForResultsForSubjectsForTeacher() {
  const [items, setItems] = useState([])
  const [answers, setAnswers] = useState([])

  useEffect(() => {
    const fetchItems = async function() {
      const contents = await getCoursePlanningsForGrade2()
      setItems(contents)
    }
    fetchItems()
  }, []);
...
export default Grade2ContentForResultsForSubjectsForTeacher
