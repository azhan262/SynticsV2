...
function Grade6ContentForResultsForSubjectsForTeacher() {
  const [items, setItems] = useState([])
  const [answers, setAnswers] = useState([])

  useEffect(() => {
    const fetchItems = async function() {
      const contents = await getCoursePlanningsForGrade6()
      setItems(contents)
    }
    fetchItems()
  }, []);
...
export default Grade6ContentForResultsForSubjectsForTeacher
