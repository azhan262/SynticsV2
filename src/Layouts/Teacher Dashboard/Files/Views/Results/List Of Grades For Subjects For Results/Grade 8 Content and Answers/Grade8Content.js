...
import { getCoursePlanningsForGrade8 } from '../../../../Apis/apiForCoursePlanning';


function Grade8ContentForResultsForSubjectsForTeacher() {
  const [items, setItems] = useState([])
  const [answers, setAnswers] = useState([])

  useEffect(() => {
    const fetchItems = async function() {
      const contents = await getCoursePlanningsForGrade8()
      setItems(contents)
    }
    fetchItems()
  }, []);
...
export default Grade8ContentForResultsForSubjectsForTeacher
