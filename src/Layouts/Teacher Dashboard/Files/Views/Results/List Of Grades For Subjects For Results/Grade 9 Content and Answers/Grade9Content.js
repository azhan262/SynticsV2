...
function Grade9ContentForResultsForSubjectsForTeacher() {
  const [items, setItems] = useState([])
  const [answers, setAnswers] = useState([])

  useEffect(() => {
    const fetchItems = async function() {
      const contents = await getCoursePlanningsForGrade9()
      setItems(contents)
    }
    fetchItems()
  }, []);
...
export default Grade9ContentForResultsForSubjectsForTeacher
